...
public class FindPatientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DB.getAllPatients(myListView,myPage,empty);
    ...
    private void validate(EditText search) {
        if(search.getText() == null || search.getText().toString().isEmpty()){

               search.requestFocus();
               search.setHint("Enter data!");
               search.setHintTextColor(Color.RED);
        }else{
          DB.search(search.getText().toString(),myListView,notFound);
        }
    }
